...
test.describe("Checkout - Event & Distance Selection", () => {
  ...
  test.describe("Distance Selection Updates", () => {
    ...
    test("should update distance facts when distance changes", async ({
      ...
      // Verify initial distance facts (234 km) - use span.font-medium to avoid matching option text
      await expect(page.locator('span.font-medium', { hasText: '234 km' })).toBeVisible();
      ...
      await expect(page.locator('span.font-medium', { hasText: '234 km' })).not.toBeVisible();
    });
